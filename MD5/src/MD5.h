...
class MD5 {
private:
	unsigned int buffer[4];//4个缓冲区，每个32位，用unsigned int存储
	string plaintext;
	unsigned char* padding_text; //padding后的数据
	unsigned long long total_len; //padding后的长度，是字节
	void resetBuffer();
	void padding();
	void run(int blocknum);
	string getResult();
...
//循环左移表
static const int LEFT_SHIFT_TABLE[4][4] = {
	{7, 12, 17, 22},
	{5, 9, 14, 20},
	{4, 11, 16, 23},
	{6, 10, 15, 21}
